(() => {
  ...
  const carrinhoAtual = new URLSearchParams(window.location.search).get("nome") || document.body.dataset.carrinho;
  const resultado = document.getElementById("resultado");
  let pedidoAtual = null;

  const tituloCarrinho = document.getElementById("tituloCarrinho");
  if (tituloCarrinho && carrinhoAtual) {
    tituloCarrinho.textContent = `CARRINHO DE ${carrinhoAtual.toUpperCase()}`;
    document.title = `Carrinho ${carrinhoAtual}`;
  }
  ...
  const nomesPlural = {
    ...
    "Massa": "Massas",
    "Doce": "Doces",
    "Cerveja lata": "Cervejas em lata",
    "Refrigerante lata": "Refrigerantes em lata",
    "Agua garrafa 350ml": "Garrafas de água 350ml",
    "Mesa": "Mesas"
  };

  function nomeProdutoComQuantidade(item) {
    const quantidade = Number(item.quantidade || 0);
    if (quantidade <= 1) return core.nomeProdutoExibicao(item.nome);
    return nomesPlural[item.nome] || `${core.nomeProdutoExibicao(item.nome)}s`;
  }
  ...
  window.consultarPedido = async () => {
    ...
    if (!pedido) {
      resultado.innerHTML = `<div class="bg-white p-6 rounded-xl shadow font-bold text-xl">Pedido não encontrado.</div>`;
      return;
    }
  ...
  function renderPedido() {
    ...
    if (itens.length === 0) {
      resultado.innerHTML = `<div class="bg-white p-6 rounded-xl shadow font-bold text-xl">Pedido não realizado neste carrinho.</div>`;
      return;
    }
    ...
    const linhas = itens.map(item => {
      ...
      return `<div class="border rounded-lg p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <p class="text-xl font-bold">${item.quantidade} ${nomeProdutoComQuantidade(item)}</p>
    ...
    resultado.innerHTML = `<div class="bg-white p-6 rounded-xl shadow">
      <h2 class="text-3xl font-black mb-2">${todosEntregues ? "PEDIDO JÁ ENTREGUE" : `Pedido ${pedidoAtual.numero}`}</h2>
