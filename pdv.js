(() => {
  ...
  function mostrarMensagemPedido(numeroPedido) {
    ...
    modal.innerHTML = `
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden">
        <div class="px-6 py-4 text-lg font-semibold border-b">Esta pagina diz</div>
        <div class="p-6">
  ...
  window.finalizarVenda = async () => {
    ...
    try {
      ...
      alert("Nao foi possivel finalizar a venda. Confira a internet e tente novamente.");
      console.error(erro);
    }
  };

  window.mostrarRelatorio = () => {
    let texto = "Relatorio de Vendas:\n\n";
    let arrecadadoTotal = 0;
    Object.entries(vendas).forEach(([nome, item]) => {
      const totalItem = item.qtd * item.valor;
      texto += `${nome}: ${item.qtd} vendidos - R$ ${core.formatarValor(totalItem)}\n`;
  ...
  window.imprimirRelatorio = () => {
    let relatorio = `<html><head><title>Relatorio de Vendas</title><style>
    ...
    <h1>Relatorio de Vendas - Festival ECIM 2026 - ${caixa.nomeExibicao}</h1>
    <table><tr><th>Produto</th><th>Quantidade</th><th>Valor Unitario</th><th>Valor Total</th></tr>`;

    let totalGeral = 0;
    Object.entries(vendas).forEach(([nome, item]) => {
      const totalItem = item.qtd * item.valor;
      totalGeral += totalItem;
      relatorio += `<tr><td>${nome}</td><td>${item.qtd}</td><td>R$ ${core.formatarValor(item.valor)}</td><td>R$ ${core.formatarValor(totalItem)}</td></tr>`;
    });
  ...
  function renderTabelaVendas() {
    ...
    produtos.forEach(produto => {
      ...
      tr.innerHTML = `<td class="border p-2">${produto.nome}</td>
        <td class="border p-2">
  ...
  window.apagarItem = async nome => {
    if (!confirm(`Deseja zerar as vendas de "${nome}"?`)) return;
